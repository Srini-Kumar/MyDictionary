...
async function fetchGeminiBilingual(word,targetLang){
  ...
  const s=getSettings();
  const prompt=`Act as a formal bilingual dictionary. For the word '${word}', provide the common English definition, and directly below it, provide the translation of that definition in ${targetLang}. Return ONLY valid JSON in this format: {"english_def": "...", "translated_def": "..."}`;
...
async function lookupWord(word){
  ...
  Store.set('mdict_last_word', word); // Saves the last searched word
    ...
    // Push searched word to browser history
  if (!isHandlingPopState) {
    history.pushState({ screen: 'home', word: word }, '', '#word=' + encodeURIComponent(word));
  }
...
function goHome(){
  ...
  document.getElementById('lang-pill-row').classList.remove('visible');
  currentWord = null;
...
function renderSettingsPage(){
  ...
  const cacheCountEl=document.getElementById('cache-count'); if(cacheCountEl) cacheCountEl.textContent=`${getCache().length} words cached`;
  updateCacheLabel();
}
...
(function(){
  try {
    ...
    window.addEventListener('popstate', function(event) {
      ...
      isHandlingPopState = true; // Prevents infinite loops with our pushState
      
      if (state && state.word) {
        document.getElementById('search-input').value = state.word;
        lookupWord(state.word);
