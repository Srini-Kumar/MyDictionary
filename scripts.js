...
/* ══════════════════════════════════════════════════
   ...
   - Secondary: Wiktionary REST API
   - Runs simultaneously. If both fail: Urban Dictionary
══════════════════════════════════════════════════ */
...
async function fetchDefinition(word) {
  ...
  try {
    // Run primary and secondary simultaneously. Return the first that succeeds.
    return await Promise.any([fetchFreeDictionary(word), fetchWiktionaryRest(word)]);
  } catch (error) {
    // Both failed, use Urban Dictionary fallback
...
/* ══════════════════════════════════════════════════
   NGRAM (Google Books + ngrams.dev as Auxiliary)
══════════════════════════════════════════════════ */
async function fetchNgram(word){
  ...
  // Function to fetch Google Books Ngram via proxy
  ...
  // Function to fetch ngrams.dev as auxiliary source
  ...
  // Run all simultaneously, return first successful result
...
async function lookupWord(word){
  ...
  try{
    ...
    if(cachedDict){data=cachedDict.data;extra=cachedDict.extra;}
    else{
      const [dictData, wikiExtra, datamuseExtra] = await Promise.all([fetchDefinition(word), fetchWikiExtra(word), fetchDatamuse(word)]);
      data = dictData;
      extra = {
        forms: wikiExtra?.forms || null,
        etymology: wikiExtra?.etymology || null,
        syns: [...(wikiExtra?.syns || []), ...(datamuseExtra?.syns || [])],
        ants: [...(wikiExtra?.ants || []), ...(datamuseExtra?.ants || [])]
      };
...
function updateQueueBtn(word){
  ...
  // Switch between Plus icon and Check icon
...
/* ══════════════════════════════════════════════════
   SIMILAR & OPPOSITE TAB (Uses Free Dict API, Wiktionary, Datamuse only)
══════════════════════════════════════════════════ */
